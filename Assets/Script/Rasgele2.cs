...
public class Rasgele2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!oyunKontrol.oyunBitis)
        {
            ...
            if (zaman < 0)
            {
                GameObject go = Instantiate(sekil[Random.Range(0, 3)], new Vector3(Random.Range(-8f, 8f), 5.85f, 0), Quaternion.Euler(0, 0, 0)) as GameObject;
                zaman = gecici;
            }
        }
        else
